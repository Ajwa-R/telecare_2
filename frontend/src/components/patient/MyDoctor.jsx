...
export default function MyDoctor({ onOpenChat, onBookAgain }) {
  ...
  useEffect(() => {
    if (!user?._id) return;
    fetch(`${API_BASE}/api/appointments/patient/${user._id}/doctors`, {
  ...
  if (!doctors.length)
    return (
      <div className="bg-white p-6 rounded-xl shadow">
        ...
        <p className="text-gray-600">No doctors yet. Book an appointment to see them here.</p>
      </div>
    );
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {doctors.map((d) => (
          <div key={d._id} className="p-4 border rounded-xl flex items-center gap-3">
            <img
              src={buildDoctorImage(d, API_BASE)}
              alt={d.name}
              className="w-14 h-14 rounded-full object-cover"
              onError={(e) => (e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(d.name||"Doctor")}`)}
            />
