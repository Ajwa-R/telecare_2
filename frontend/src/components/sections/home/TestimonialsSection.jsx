...
//..
  const load = () =>
    fetch(`${API}/api/reviews/public?limit=8`)
      .then((r) => r.json())
      .then((d) => {
        setItems(d.items || []);
...
useEffect(() => {
  ...
  if (!token) return;         // abhi login hua hi nahi
  ...
  (async () => {
    ...
    try {
      ...
      alert("Thanks! Your review has been submitted. It will appear after admin approval.");
      setShowForm(false);
      setText(""); setCity(""); setRating(5);
      load();
    } catch (e) {
...
//..
  const submit = async () => {
    ...
    //   try { navigate("/login?redirect=/"); } catch {}
    //   return;
    // }
    if (!token) {
     // Draft save karo taake login ke baad auto-submit ho jaye
    ...
    try {
      const res = await fetch(`${API}/api/reviews`, {
        ...
        body: JSON.stringify({ rating, text, city }),
      });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body?.message || `HTTP ${res.status}`);

      setText(""); setCity(""); setRating(5); setShowForm(false);
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600 mt-2">Real stories from people who trusted TeleCare.</p>
        <div className="text-sm text-gray-600 mt-2">
          ⭐ {Number(stats.avg || 0).toFixed(1)} average • {stats.count || 0} reviews
        </div>
      </div>
        ...
        {!showForm ? (
          <button
            ...
            className={`px-6 py-2 rounded-full shadow-lg text-white ${
              isAdmin ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
            }`}
          >
          ...
          <div className="w-full max-w-xl bg-white rounded-2xl p-4 shadow border">
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((i) => (
                  <button key={i} onClick={() => setRating(i)} className="focus:outline-none">
                    <FaStar className={i <= rating ? "text-yellow-400" : "text-gray-300"} />
                  </button>
                ))}
              </div>
              <input
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => setShowForm(false)} className="px-3 py-2 rounded bg-gray-100">
                Cancel
              </button>
              <button onClick={submit} disabled={saving} className="px-4 py-2 rounded bg-emerald-600 text-white">
