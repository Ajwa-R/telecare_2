...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { name, email, password, gender, age, specialization, role, experience } = form;
    ...
    if (role === "doctor" && (!specialization || !experience)) {
      alert("Doctor fields are required (specialization, experience).");
      return;
    }

    // JSON payload (no image upload)
    const payload = { name, email, password, gender, age, role };
    if (role === "doctor") Object.assign(payload, { specialization, experience });

    try {
      const res = await api.post("/auth/register", payload);
      alert("Registered: " + (res?.user?.name || name));
  ...
  const invalid = (v) => (!v && attemptedSubmit ? "border-red-500" : "border-gray-300");
    ...
    <div className="min-h-screen bg-gradient-to-r from-green-400 to-blue-500 flex items-center justify-center px-4 py-10">
      <div className="bg-white rounded-xl shadow-2xl grid grid-cols-1 md:grid-cols-2 w-full max-w-5xl overflow-hidden">
        <div className="bg-blue-50 flex flex-col items-center justify-center p-10">
          ...
          <h2 className="text-xl font-bold text-blue-600">Join TeleCare Today</h2>
        </div>
        ...
        >
          ...
          <input name="name" onChange={handleChange} placeholder="Full Name"
                 className={`border px-4 py-2 w-full rounded focus:outline-blue-500 ${invalid(form.name)}`} />

          <input type="email" name="email" onChange={handleChange} placeholder="Email"
                 className={`border px-4 py-2 w-full rounded focus:outline-blue-500 ${invalid(form.email)}`} />

          <div className="relative">
            <input type={showPassword ? "text" : "password"} name="password" onChange={handleChange}
                   placeholder="Password"
                   className={`border px-4 py-2 w-full rounded focus:outline-blue-500 ${invalid(form.password)}`} />
            <span onClick={togglePassword} className="absolute right-3 top-2.5 cursor-pointer text-gray-500">
              {showPassword ? "🙈" : "👁️"}
            </span>
          </div>
          ...
          <select name="gender" onChange={handleChange}
                  className={`border px-4 py-2 w-full rounded focus:outline-blue-500 ${invalid(form.gender)}`}>
          ...
          <input type="number" name="age" onChange={handleChange} placeholder="Age"
                 className={`border px-4 py-2 w-full rounded focus:outline-blue-500 ${invalid(form.age)}`} />
